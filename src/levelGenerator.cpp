...
Stage *baseGenerator() {
    ...
    Entity *player = new Entity("Player", 500, 500, 10 * SCALE_FACTOR, 10 * SCALE_FACTOR, Player, 50, 10, 100, 10);
    player->following = true;
    stage->eManager.entities.push_back(floor);
    stage->eManager.entities.push_back(player);

    int numGrass = (rand() % 300) + 150;
    for (int i = 0; i < numGrass; ++i) {
        ...
        tGrass->attributes.push_back(NilSimulation);
        stage->eManager.entities.push_back(tGrass);
    }

    int numBirds = rand() % 50;
    for (int i = 0; i < numBirds; ++i) {
        Entity *bird = new Entity("Bird", rand()%(floorWidth-(32 * (SCALE_FACTOR / 5))), rand()%(floorHeight-(32 * (SCALE_FACTOR / 5))), 16 * (SCALE_FACTOR / 5), 16 * (SCALE_FACTOR / 5), Bird, 5, 3, 10, 25);
    ...
    for (int i = 0; i < numCats; ++i) {
        Entity *cat = new Entity("Cat", rand()%(floorWidth-(32 * (SCALE_FACTOR / 5))), rand()%(floorHeight-(32 * (SCALE_FACTOR / 5))), 16 * (SCALE_FACTOR / 5), 16 * (SCALE_FACTOR / 5), Cat, 15, 5, 20, 12);
